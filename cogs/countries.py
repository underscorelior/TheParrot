...
class Countries(commands.Cog):
    ...
    async def leaderboard(self, ctx):
        ...
        channel = get(guild.text_channels, topic=str(
            "Country quiz, new game starts every 15 seconds! https://github.com/underscorelior/TheParrot"))
    ...
    async def countries(self):
        ...
        if (x - datetime.utcnow().timestamp()) <= -200:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get("https://underscore.wtf/countries/countries.json",
                                       ssl=False) as r: data = await r.json()
                async with session.get("https://komali.dev/bin/territories.json",
                                       ssl=False) as f: altdata = await f.json()
            ...
            if t == 1 and ccfixed:
                t = "capital"
                if quizans["capital"] == "City of San Marino":
                    quizans["capital"] = qex = "San Marino"
                else:
                    qex = ccfixed
                msem = discord.Embed(title=f'What is the capital of `{quizans["name"]}`:', color=0x1860cc,
                ...
                if random.randint(1, 17) == 7:
                    ...
                    tc = 2
                t = "name"
                msem = discord.Embed(title=f'Which {"country" if tc == 2 else "territory"} does this flag belong to?',
                                     color=0x1860cc, timestamp=datetime.utcnow()).set_image(url=quizans["flags"])
            ...
            def check(message: discord.Message) -> bool:
                ...
                if content == "nmi": content = "Northern Marianas"
                if content == "spm": content = "Saint Pierre and Miquelon"
                if content == "sjk": content = "Sri Jayawardenepura Kotte"
                return message.channel == channel and message.author != self.bot and (
                        unidecode(content).lower().replace("-", " ").replace("'", "") == unidecode(
                    qqqq).lower().replace("-", " ").replace("'", ""))

            try:
                message = await self.bot.wait_for('message', timeout=12.5, check=check)
            except asyncio.TimeoutError:
                if t == "capital":
                    await em.edit(embed=discord.Embed(title="No one answered correctly!",
                                                      description=f'What is the capital of `{quizans["name"]}:` \nReal Answer: `{quizans["capital"]}`',
                                                      color=0xfa8e23, timestamp=datetime.utcnow()))
                else:
                    await em.edit(embed=discord.Embed(title="No one answered correctly!",
                                                      description=f'Which {"country" if tc == 2 else "territory"} does this flag belong to?: \nReal Answer: `{quizans["name"]}`',
                                                      color=0xfa8e23, timestamp=datetime.utcnow()).set_thumbnail(
                        url=quizans["flags"]))
...
```""")
                ...
                if t == "capital":
                    await em.edit(embed=discord.Embed(title=f'{message.author} answered correctly!',
                                                      description=f'What is the capital of `{quizans["name"]}`: \nAnswer: `{quizans["capital"]}`',
                                                      color=0x3cb556, timestamp=datetime.utcnow()).set_author(
                        name=message.author, icon_url=message.author.avatar_url).set_footer(
                        text=f"They have a total of {totals} point(s)!"))
                else:
                    await em.edit(embed=discord.Embed(title=f'{message.author} answered correctly!',
                                                      description=f'Which {"country" if tc == 2 else "territory"} does this flag belong to? \nAnswer: `{quizans["name"]}`',
                                                      color=0x3cb556, timestamp=datetime.utcnow()).set_thumbnail(
                        url=quizans["flags"]).set_author(name=message.author,
                                                         icon_url=message.author.avatar_url).set_footer(
                        text=f"They have a total of {totals} point(s)!"))
    ...
    async def quiz(self, ctx):
        if ctx.channel.id == 955169257711370280:
            ...
            btnans = [[Button(label=ca[0], emoji="🇦", style=ButtonStyle.blue, custom_id=1),
            ...
            if qtype == 1:
                msem = discord.Embed(title=f'What is the capital of `{quizans["name"]}`:', color=0x1860cc,
                                     timestamp=datetime.utcnow()).set_footer(text=ctx.author,
                                                                             icon_url=ctx.author.avatar_url)
            else:
                msem = discord.Embed(title=f'Which country does this flag belong to?', color=0x1860cc,
                                     timestamp=datetime.utcnow()).set_image(url=quizans["flags"]).set_footer(
            ...
            try:
                ansch = await self.bot.wait_for('button_click', check=lambda
                    inter: inter.message.id == message.id and inter.user.id == ctx.author.id, timeout=15)
            except asyncio.TimeoutError:
                if qtype == 1:
                    return await message.edit(embed=await toembed(
                        f'What is the capital of `{quizans["name"]}`: \nAnswer: `{quizans["capital"]}`'), components=[
                        [Button(emoji="🇦", style=ButtonStyle.grey, disabled=True),
                         Button(emoji='🇧', style=ButtonStyle.grey, disabled=True),
                         Button(emoji="🇨", style=ButtonStyle.grey, disabled=True),
                         Button(emoji='🇩', style=ButtonStyle.grey, disabled=True)]])
                else:
                    return await message.edit(
                        embed=await toembedt(f'Which country does this flag belong to? \nAnswer: `{quizans["name"]}`',
                                             quizans["flags"]), components=[
                            [Button(emoji="🇦", style=ButtonStyle.grey, disabled=True),
                             Button(emoji='🇧', style=ButtonStyle.grey, disabled=True),
                             Button(emoji="🇨", style=ButtonStyle.grey, disabled=True),
                             Button(emoji='🇩', style=ButtonStyle.grey, disabled=True)]])
            if int(ansch.custom_id) == ansloc:
                ...
                _save()
                if qtype == 1:
                    await message.edit(embed=discord.Embed(title='Win',
                                                           description=f'What is the capital of `{quizans["name"]}`: \nAnswer: `{quizans["capital"]}`',
                                                           color=0x3cb556, timestamp=datetime.utcnow()).set_footer(
                        text=f'{ctx.author} | {amounts[str(ctx.author.id)]} Point(s)', icon_url=ctx.author.avatar_url),
                        components=await winbtn(ansloc))
                else:
                    await message.edit(embed=discord.Embed(title='Win',
                                                           description=f'Which country does this flag belong to? \nAnswer: `{quizans["name"]}`',
                                                           color=0x3cb556, timestamp=datetime.utcnow()).set_footer(
                        text=f'{ctx.author} | {amounts[str(ctx.author.id)]} Point(s)',
                        icon_url=ctx.author.avatar_url).set_thumbnail(url=quizans["flags"]),
                                       components=await winbtn(ansloc))
            else:
                qta = await losebtn(int(ansch.custom_id), ansloc)
                if qtype == 1:
                    await message.edit(embed=discord.Embed(title='Lose',
                                                           description=f'What is the capital of `{quizans["name"]}`: \nSelected Answer: `{btnans[0][qta[1] - 1].label}` \nReal Answer: `{quizans["capital"]}`',
                                                           color=0xfa8e23, timestamp=datetime.utcnow()).set_footer(
                        text=f'{ctx.author} | {amounts[str(ctx.author.id)]} Point(s)', icon_url=ctx.author.avatar_url),
                        components=qta[0])
                else:
                    await message.edit(embed=discord.Embed(title='Lose',
