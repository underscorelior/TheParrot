	...
	async def report(self,ctx):
		try:  
			...
			if repmessage.author.id == ctx.author.id: await ctx.author.send("You cant report yourself!"); await ctx.message.delete()
				...
				repcon=[[Button(emoji="✅", style=ButtonStyle.green, custom_id='y'),Button(emoji='⛔', style=ButtonStyle.red, custom_id='n')]]; await message.edit(content='Are you sure you want to submit the report?',components=repcon)
				try: conin = await self.bot.wait_for('button_click',check=lambda inter: inter.message.id == message.id,timeout=30)
				except asyncio.TimeoutError: await message.delete(); await ctx.message.delete(); return await ctx.author.send(content='Timed out!')
				if conin.custom_id == 'y':
					repcat=""
					if "S" in interaction.values: repcat += "🤬 Swearing/Bad Language    "
					if "N" in interaction.values: repcat += "🔞 NSFW/GORE    "
					...
					try: embed=discord.Embed(title=f"{repmessage.author} was reported!", description=repmessage.content, color=0xd42121,url=repmessage.jump_url).set_image(url=imgs).set_author(name=f"Reported by {ctx.author}", icon_url=ctx.author.avatar_url).set_footer(text=f"Report categories: {repcat}"); repmods = await ch.send(embed=embed,components=modsel)
					except HTTPException:embed=discord.Embed(title=f"{repmessage.author} was reported!", description=repmessage.content, color=0xd42121,url=repmessage.jump_url).set_author(name=f"Reported by {ctx.author}", icon_url=ctx.author.avatar_url).set_footer(text=f"Report categories: {repcat}");repmods = await ch.send(embed=embed,components=modsel)
					repmodct = await self.bot.wait_for('button_click',check=lambda inter: inter.message.id == repmods.id)
					if repmodct.custom_id == "d":
						for row in modsel: row.disable_components();await repmods.edit(content=f"Message deleted. \nActioned by {repmodct.author}",components=modsel);await repmessage.delete();await ctx.author.send("Your report has been dealt with.")
					if repmodct.custom_id == "f":
						for row in modsel:row.disable_components();await repmods.edit(content=f"No Action. \nActioned by {repmodct.author}",components=modsel);await ctx.author.send("Your report has been dealt with.")
